...
namespace BlockchainAssignment
{
    class Blockchain
    {
        ...
        {
            String rehash = b.CreateHash((int)b.nonce, new List<Transaction>());
        ...
        // Select transactions from the pool based on a selection method
        public List<Transaction> SelectTransactionsFromPool(string selectionMethod, string walletAddress = "")
        {
            ...
            switch (selectionMethod)
            {
                case "Greedy":
                    ...
                    var sortedPool = transactionPool.OrderByDescending(t => t.fee);
                    selectedTransactions = sortedPool.Take(6).ToList();
                    break;

                case "Altruistic Mining":
                    // Sort transactions by timestamp in ascending order
                    selectedTransactions = transactionPool.OrderBy(t => t.timestamp).Take(6).ToList();
                    break;

                case "Random":
                    // Select transactions randomly from the pool
                    Random random = new Random();
                    int transactionIndex = random.Next(0, transactionPool.Count);
                    selectedTransactions.Add(transactionPool[transactionIndex]);
                    break;

                case "Address Based":
                    // Select transactions from the pool that contain a specific wallet address
                    selectedTransactions = transactionPool.Where(t => t.senderAddress == walletAddress || t.recipientAddress == walletAddress)
                                                          .Take(6).ToList();
                    break;
